...
<div class="row shadow  mb-4 container-fluid">
	<div class="col-lg-11">
		<h1 class="h3 mb-2 text-gray-800">Subphotography</h1>
	</div>
	<div class=" col-lg-1 ">            
		<a href="{{route('subphotographies.create')}} "> <button type="button" class="btn btn-success active">AddNew</button></a>
	</div>
</div>
<div class="card-table ">
	<div class="table-responsive">
		...
		<table class="table table-bordered" id="dataTable" cellspacing="0" width="100%">
			<thead>
				<tr>
					<th>No</th>
					<th>Subphotography Name</th>
					<th>Photography_id</th>
					<th>Photo</th>		
					<th>Actions</th>			
				</tr>
			</thead>
	...
	<tbody>
                ...
                @foreach($subphotography as $row)
				<tr>
					<td>{{$i++}} </td>
					<td>{{$row->name}}</td>
					<td>{{$row->photography_id}}</td>
					<td><img src="{{asset($row->photo)}}" class="img-fluid w-25" width="100" height="100"></td>
					<td><a href="{{route('subphotographies.edit',$row->id)}} " class="btn btn-warning">Edit</a>
						<form method="post" action="{{route('subphotographies.destroy',$row->id)}} " onsubmit="return confirm('Are you sure?')">
							@csrf
							@method('DELETE')
